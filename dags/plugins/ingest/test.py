from fabric import FabricIngestEngine
from postgresql import PostgresqlIngestEngine
from write import WriteEngine
from config import EMPIRE_CONNECTION_URL,LAKEHOUSE_URL,LAKEHOUSE_S3_ACCESS_KEY,LAKEHOUSE_S3_SECRET_KEY,LAKEHOUSE_S3_BUCKET,POSTGRESQL_CONNECTION_URL
...
def test2():
    ...
    iceberg_write_engine.test_create_namespace("staging")
...
get_postgresql_table(schema_name="staging", table_name="referentiedata")

# #################################################################################
# INSTRUCTION: kubectl -n hive port-forward svc/hive-metastore 9083:9083
# python dags/plugins/ingest/test.py
# #################################################################################
...
# from ingest_engine import FabricIngestEngine
...
#         engine = FabricIngestEngine(
#             odbc_conn_string="Driver={ODBC Driver 18 for SQL Server};Server=...;",
#             s3_endpoint="s3.example.nl",
#             s3_access_key="XXX",
#             s3_secret_key="YYY",
#             bucket="datalake"
#         )

#         engine.ingest_partitioned(table_name, partitions=6)

#     tables = ["orders", "customers", "invoices"]

#     ingest_table.expand(table_name=tables)
